...
if($Operation -eq "onboard" -And $storageAccountName -ne "None")
{
    ...
    if($OperationExitCode -notIn $nonOffboardExitCodes)
    {
        ...
        . $pythonExe .\appliance_setup\run.py "collect-logs" $FilePath $LogLevel $isAutomated $storageAccountName $getArcApplianceLogs $ManagedIdentityResourceId
        . $pythonExe .\appliance_setup\run.py "offboard" $FilePath $LogLevel $isAutomated
    }
}
